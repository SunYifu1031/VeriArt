...
from app.models import QAResponse, Citation, GraphPayload, Node, ChatMessage
...
async def answer(query: str, history: list[ChatMessage] | None = None) -> QAResponse:
    store = get_kg_store()
    q = (query or "").strip()
    if not q:
        return QAResponse(
    ...
    hits, _ = store.search_nodes(q, page=1, page_size=20)
    ...
    answer_text = await generate_answer(SYSTEM_PROMPT, user_message, history=history_dicts)
    if answer_text:
        return QAResponse(
            answer=answer_text,
            citations=citations,
